...
class CustomDataset(Dataset):
    def __init__(self, img_path: list, label_path: list, img_size=640, augment=False, hyp=None, radam_perspect=False):
        '''
    ...
    def __getitem__(self, index):
        ...
        img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)  # img, ratio, (dw, dh)
        ...
        if self.augment:
            ...
            # Augment imagespace
            if self.radam_perspect:  # default: not use random_perspective fucn, radam_perspect is False
        ...
        if self.augment:
            ...
            if random.random() < hyp['fliplr']:  # default: fliplr is 0.5, 50% probability to fliplr
                img = np.fliplr(img)
                if nL:
                    labels[:, 1] = 1 - labels[:, 1]
            # # for Visualize
...
if __name__ == '__main__':
    ...
    # CustomDataset
    dataset = CustomDataset(train_img_path, train_label_path, augment=True, hyp=hyp, radam_perspect=False)
