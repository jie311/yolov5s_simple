...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # cfg: yaml model file. nc && anchor relay cfg
...
def parse_model(d, ch):
    print('\n------- model layer info -------')
    print('\n%3s%18s%3s%10s  %-40s%-30s%10s%10s' % ('', 'from', 'n', 'params', 'module', 'arguments', 'cin', 'cout')
          )  # print param cfg
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        print('%3s%18s%3s%10.0f  %-40s%-30s%10s%10s' % (i, f, n, np, t, args, c1, c2))  # print param cfg
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if
                    x != -1)  # append to savelist: the from is not -1 in the yaml cofig，be Concat与Detect
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
...
def model_info(model, verbose=False, img_size=640):
    print("\n------- verbose:{}, img_size:{} -------\n------- model detail info -------".format(verbose, img_size))
    ...
    try:  # FLOPS
        ...
        flops = profile(copy.deepcopy(model), inputs=(img,), verbose=False)[0] / 1E9 * 2  # stride GFLOPS
