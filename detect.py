...
def detect(opt):
    source, weights, imgsz, data = opt.source, opt.weights, opt.img_size, opt.data
    ...
    for i, det in enumerate(pred):  # detections per image
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                cv2.rectangle(im0, c1, c2, (0, 255, 255), thickness=1, lineType=cv2.LINE_AA)
                cv2.putText(im0, label, (c1[0], c1[1] - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), thickness=1,
